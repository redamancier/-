...
void Fhan_LQR(LQR *LQR_INPUT, float expect_LQR) {
    ...
    LQR_INPUT->LQRh0 =
        LQR_INPUT->LQRN0 *
        LQR_INPUT->LQRh; //用h0代替h，解决最速跟踪微分器速度超调问题
    ...
    LQR_INPUT->LQRfh = -LQR_INPUT->LQRr * (a / d) * Fsg_LQR(a, d) -
                       LQR_INPUT->LQRr * Sign_LQR(a) *
                           (1 - Fsg_LQR(a, d)); //获得最速微分加速度跟踪量
    LQR_INPUT->LQRx1 += LQR_INPUT->LQRh * LQR_INPUT->LQRx2; // x1状态量
    LQR_INPUT->LQRx2 += LQR_INPUT->LQRh * LQR_INPUT->LQRfh; // x2状态量
}

void BodyRateControl(LQR *LQR_InputWx, LQR *LQR_InputWy, LQR *LQR_InputWz,
                     float WxExpect, float WxEstimate, float WyExpect,
                     float WyEstimate, float WzExpect, float WzEstimate) {
    /**** 	TD跟踪器 实现设定值的微分提取  ****/
    ...
    /**** 	数据的叉乘  ****/
    //	LQR_InputWx->n_error = LQR_InputWy->LQRx1 * (LQR_InputWz->inertia *
    //LQR_InputWz->LQRx1)
    //																		- (LQR_InputWy->inertia *
    //LQR_InputWy->LQRx1) * LQR_InputWz->LQRx1
    //																						+ LQR_InputWx->inertia *
    //LQR_InputWx->LQRx2 - UAVThrust.body_torquesX;

    //	LQR_InputWy->n_error = -(LQR_InputWx->LQRx1 * (LQR_InputWz->inertia *
    //LQR_InputWz->LQRx1)
    //																		- (LQR_InputWx->inertia *
    //LQR_InputWx->LQRx1) * LQR_InputWz->LQRx1)
    //																						+ LQR_InputWy->inertia *
    //LQR_InputWy->LQRx2 - UAVThrust.body_torquesY;

    //	LQR_InputWz->n_error = LQR_InputWx->LQRx1 * (LQR_InputWy->inertia *
    //LQR_InputWy->LQRx1)
    //																		- (LQR_InputWx->inertia *
    //LQR_InputWx->LQRx1) * LQR_InputWy->LQRx1
    //																						+ LQR_InputWz->inertia *
    //LQR_InputWz->LQRx2 - UAVThrust.body_torquesZ;
    /**** 产生实际的力矩 ****/
    UAVThrust.wxThrust = (LQR_InputWx->Kw * LQR_InputWx->w_error +
                          LQR_InputWx->kn * LQR_InputWx->n_error +
                          WyEstimate * (LQR_InputWz->inertia * WzEstimate) -
                          (LQR_InputWy->inertia * WyEstimate) * WzEstimate +
                          0.0002f * LQR_InputWx->LQRx2); //

    UAVThrust.wyThrust =
        (LQR_InputWy->Kw * LQR_InputWy->w_error +
         LQR_InputWy->kn * LQR_InputWy->n_error +
         (-(WxEstimate * (LQR_InputWz->inertia * WzEstimate) -
            (LQR_InputWx->inertia * WxEstimate) * WzEstimate)) +
         0.0002f * LQR_InputWy->LQRx2); //

    UAVThrust.wzThrust = (LQR_InputWz->Kw * LQR_InputWz->w_error +
                          LQR_InputWz->kn * LQR_InputWz->n_error +
                          WxEstimate * (LQR_InputWy->inertia * WyEstimate) -
                          (LQR_InputWx->inertia * WxEstimate) * WyEstimate +
...
void MotorThrustCallback(float Accelerator1, float Accelerator2,
                         float Accelerator3, float Accelerator4,
                         float arm_length, float rotor_drag_coeff) {

    const double f1 = 5.8629f * Accelerator1 * Accelerator1 -
                      0.4797f * abs(Accelerator1) + 0.6661f;
    const double f2 = 5.8629f * Accelerator2 * Accelerator2 -
                      0.4797f * abs(Accelerator2) + 0.6661f;
    const double f3 = 5.8629f * Accelerator3 * Accelerator3 -
                      0.4797f * abs(Accelerator3) + 0.6661f;
    const double f4 = 5.8629f * Accelerator4 * Accelerator4 -
                      0.4797f * abs(Accelerator4) + 0.6661f;
...
void MotorThrust(float f1, float f2, float f3, float f4) {
    ...
    if (Throttle_Info.M1 > 800)
        Throttle_Info.M1 = 800;
    if (Throttle_Info.M2 > 800)
        Throttle_Info.M2 = 800;
    if (Throttle_Info.M3 > 800)
        Throttle_Info.M3 = 800;
    if (Throttle_Info.M4 > 800)
        Throttle_Info.M4 = 800;

    if (Throttle_Info.M1 < 100)
        Throttle_Info.M1 = 100;
    if (Throttle_Info.M2 < 100)
        Throttle_Info.M2 = 100;
    if (Throttle_Info.M3 < 100)
        Throttle_Info.M3 = 100;
    if (Throttle_Info.M4 < 100)
        Throttle_Info.M4 = 100;

    PWM_OUTPUT(Throttle_Info.M1, Throttle_Info.M2, Throttle_Info.M3,
               Throttle_Info.M4);
}

void PWM_OUTPUT(unsigned int Motor1, unsigned int Motor2, unsigned int Motor3,
                unsigned int Motor4) {
    Motor1 += 1000;
    Motor2 += 1000;
    Motor3 += 1000;
