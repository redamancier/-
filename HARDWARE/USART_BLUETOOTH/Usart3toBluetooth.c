 ...
 ************************* 作者 	:Xiluna Tech 文件名 :Usart3toBluetooth.c
 * 描述   :蓝牙串口配置
 * 官网   :http://xiluna.com/
 * 公众号 :XilunaTech
 **********************************************************************************/
...
void Usart3toBluetooth_Init(u32 Bound) {
    ...
    GPIO_PinAFConfig(GPIOB, GPIO_PinSource10,
                     GPIO_AF_USART3); // GPIOC10复用为USART3
    GPIO_PinAFConfig(GPIOB, GPIO_PinSource11,
                     GPIO_AF_USART3); // GPIOC11复用为USART3
...
void inf_Uart3_deal_irq_dma_tx(void) { _Uart3_deal_irq_dma_tx(); }

uint8_t inf_Uart3_deal_irq_tx_end(void) { return _Uart3_deal_irq_tx_end(); }

uint8_t inf_Uart3_deal_irq_rx_end(uint8_t *buf) {
    return _Uart3_deal_irq_rx_end(buf);
}

void Uart3_dma_tx_irq_handler(void) { inf_Uart3_deal_irq_dma_tx(); }
